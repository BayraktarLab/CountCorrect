...
class ProbeCounts_GeneralModel_V2(Pymc3Model):
    ...
    - a parameter to model sequencing depth
    - a parameter to model batch effects
    ...
    def __init__(
        ...
        mu_l_v_g = 0.01, # Controls how much extra variance over Poisson we expect in the NB distribution (higher value = more extra variance)
        v_mu_e_g = 0.25, # controls how much we want to rely on prior knowledge of background binding (negative probe counts), so higher values = more reliance on prior knowledge
        detection_alpha_hyp = 0.1 # Controls how much variance in detection efficiency we expect between ROIs. Higher values = less variance
        ):
        ...
        with self.model:
            ...
            ### Expected number of nuclei per roi (maybe add noise to this later) ###
            ...
            # A normalized expression measure may be obtained from the product of NMF and expression level:
            self.A_rg_norm =  pm.Deterministic('A_rg_norm', pm.math.dot(self.w_rf, self.h_gf.T)*self.e_g)
            
            ## Global and gene specific batch effects on each probe:
            ...
            self.gene_level_e = pm.Gamma('gene_level_e', 20,
                                         20, shape=(self.n_exper, 1))
            
            ## location specific effects on probes:
            ...
            self.detection_alpha_hyp = pm.Deterministic(
                "detection_alpha_hyp", theano.shared(detection_alpha_hyp) * tt.ones((self.n_exper, 1))
            )
            ...
            # A_rg_norm multiplied with nuclei counts in each ROI defines gene expression in each ROI:
            self.A_rg =  pm.Deterministic('A_rg', tt.extra_ops.repeat(self.nuclei_r.T, self.n_genes, axis=1) * self.A_rg_norm)
            
            # Gene counts expected value is sum of gene expression and background times batch effects, location specific effects:
            self.x_rg =  pm.math.dot(self.extra_data_tt['spot2sample'], self.gene_level_e) \
            * pm.math.dot(self.extra_data_tt['spot2sample'], self.gene_level_independent) \
            * self.detection_eff_y_s * (self.A_rg + self.B_rg)
            ...
            self.y_rn = self.detection_eff_y_s*self.b_n*self.l_r
    ...
    def compute_X_corrected(self):
        r""" Save expected value of specific and non-specific binding"""
        self.X_latent_mean = np.round(self.samples['post_sample_means']['A_rg_norm'])
        self.X_latent_q05 = np.round(self.samples['post_sample_q05']['A_rg_norm'])
        self.X_latent_q95 = np.round(self.samples['post_sample_q95']['A_rg_norm'])  
        self.X_latent_q01 = np.round(self.samples['post_sample_q01']['A_rg_norm'])
        self.X_latent_q99 = np.round(self.samples['post_sample_q99']['A_rg_norm'])
...
#             subset = fraction_removed > 0
            ...
            removed_counts_ISC_gene = removed_counts_ISC_gene
            total_counts_gene = total_counts_gene
            fraction_removed = fraction_removed

            geneIndex_to_annotate = np.squeeze([np.where(self.var_names == genesOfInterest[i])
                                            for i in range(len(genesOfInterest))])
            SMALL_SIZE = 26
            MEDIUM_SIZE = 26
            BIGGER_SIZE = 26
...
#             ax.set_ylim(ymin, ymax)
            ...
            ax.legend()
            ax.set_xlabel('Mean counts across ROIs')
            ax.set_ylabel('Mean counts added by CountCorrect')
            ax.set_facecolor('white')
            ax=plt.gca()
            cbar = fig.colorbar(mesh)
            cbar.set_label('Fraction Added')
    ...
    def rank_X_corrected_genes(self):
        ...
        cpm = self.X_data/total_counts_corrected*10**(6)
        total_counts1 = np.sum(self.X_corrected_mean, axis = 0)
        cpm_corrected1 = self.X_corrected_mean
