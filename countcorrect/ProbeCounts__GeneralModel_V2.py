...
class ProbeCounts_GeneralModel_V2(Pymc3Model):
    ...
    - a parameter to model batch effects, between different slides
    ...
    def __init__(
        ...
        obs_names=None, fact_names=None, slide_id=None,
        n_factors = 30,
        ...
        # generate one-hot encoded parameters for slides
        self.spot2slide_df = pd.get_dummies(slide_id)
        # convert to np.ndarray
        self.spot2slide_mat = self.spot2slide_df.values
        self.n_exper = self.spot2slide_mat.shape[1]
        # assign extra data to dictionary with (1) shared parameters (2) input data
        self.extra_data_tt = {'spot2slide': theano.shared(self.spot2slide_mat.astype(self.data_type))}
        self.extra_data = {'spot2slide': self.spot2slide_mat.astype(self.data_type)}
        ...
        with self.model:
            ...
            self.x_rg =  pm.math.dot(self.extra_data_tt['spot2slide'], self.gene_level_e) \
            * pm.math.dot(self.extra_data_tt['spot2slide'], self.gene_level_independent) \
            * (self.A_rg + self.B_rg)
